...
func vote(hm *VotingResultManager, source string, height, round uint64) {
	b := *ballot.NewBallot(source, source, voting.Basis{Height: height, Round: round}, []string{})
...
func TestUpdate(t *testing.T) {
	...
	p, err := NewDefaultVotingThresholdPolicy(67)
	require.NoError(t, err)

	p.SetValidators(len(sources))
	hm := NewVotingResultManager(p)

	vote(hm, sources[0], 9, 0)
	vote(hm, sources[0], 10, 0)
	vote(hm, sources[1], 10, 0)

	require.Equal(t, 2, len(hm.recentVoting))
	require.Equal(t, 1, len(hm.votingResults))

	vote(hm, sources[2], 10, 0)
	vote(hm, sources[3], 10, 0)

	require.Equal(t, 4, len(hm.recentVoting))
	require.Equal(t, 1, len(hm.votingResults))

	vote(hm, sources[2], 10, 1)
	vote(hm, sources[3], 10, 1)

	require.Equal(t, 4, len(hm.recentVoting))
	require.Equal(t, 2, len(hm.votingResults))

	vote(hm, sources[0], 10, 1)
	vote(hm, sources[1], 10, 1)

	require.Equal(t, 1, len(hm.votingResults))

	vote(hm, sources[0], 9, 10)
	require.Equal(t, 4, len(hm.recentVoting))
	require.Equal(t, 1, len(hm.votingResults))
}
...
func TestGetSyncInfoNormal(t *testing.T) {
	...
	p.SetValidators(len(valids) + len(invalids))
	hm := NewVotingResultManager(p)

	vote(hm, valids[0], 10, 0)
	vote(hm, valids[1], 10, 0)
	vote(hm, valids[2], 10, 0)
	vote(hm, invalids[0], 10, 0)
	vote(hm, valids[3], 10, 0)

	b := *ballot.NewBallot(valids[3], valids[3], voting.Basis{Height: 10, Round: 0}, []string{})
	b.SetVote(ballot.StateACCEPT, voting.YES)

	height, nodeAddrs, err := hm.getSyncInfo(b)
	require.NoError(t, err)
...
func TestGetSyncInfoFoundSmallestHeight(t *testing.T) {
	...
	p.SetValidators(len(nodes))
	hm := NewVotingResultManager(p)

	vote(hm, nodes[0], 32, 0)
	vote(hm, nodes[1], 33, 0)
	vote(hm, nodes[2], 33, 0)
	vote(hm, nodes[3], 34, 0)
	vote(hm, nodes[4], 35, 0)
	...
	vote(hm, nodes[0], 36, 0)
	vote(hm, nodes[1], 36, 0)
	vote(hm, nodes[2], 36, 1)
	vote(hm, nodes[3], 36, 1)
	vote(hm, nodes[4], 36, 1)
