...
export NODE1=$(docker run -d --network host --env-file=${ROOT_DIR}/docker/node1.env \
                        ${NODE_IMAGE} node --log-level=debug --rate-limit-api=0-s --rate-limit-node=0-s --threshold=60 \
...
export NODE4=$(docker run -d --network host --env-file=${ROOT_DIR}/docker/node4.env \
                        ${NODE_IMAGE} node --log-level=debug --rate-limit-api=0-s --rate-limit-node=0-s --threshold=60 \
                        --genesis=${SEBAK_GENESIS},${SEBAK_COMMON},5000000000000000 --block-time-delta=${BLOCK_TIME_DELTA})
export NODE5=$(docker run -d --network host --env-file=${ROOT_DIR}/docker/node5.env \
                        ${NODE_IMAGE} node --log-level=debug --rate-limit-api=0-s --rate-limit-node=0-s --threshold=60 \
                        --genesis=${SEBAK_GENESIS},${SEBAK_COMMON},5000000000000000 --block-time-delta=${BLOCK_TIME_DELTA})
...
do
    ...
    # Rerun NODE5 for checking sync
    docker stop ${NODE5}
    docker rm -f ${NODE5}

    export NODE5=$(docker run -d --network host --env-file=${ROOT_DIR}/docker/node5.env \
                            ${NODE_IMAGE} -test.coverprofile=coverage.txt \
                            node --log-level=debug --rate-limit-api=0-s --rate-limit-node=0-s --threshold=60 \
                            --genesis=${SEBAK_GENESIS},${SEBAK_COMMON},5000000000000000 --block-time-delta=${BLOCK_TIME_DELTA})

    CONTAINERS="${CONTAINERS} ${NODE5}"
...
mkdir -p ./long-term/coverage/node{1,2,3,4,5}/
...
docker cp ${NODE5}:/sebak/coverage.txt ./long-term/coverage/node5/coverage.txt

# Cleanup
docker rm -f ${NODE1} ${NODE2} ${NODE3} ${NODE4} ${NODE5} || true
